...
class Site {
  ...
  static initMoveTop() {
    ...
    if (!this.empty(buttons)) buttons.forEach(button => {
  ...
  static initModals(selector) {
    ...
    if (mdl_close && typeof mdl_close !== false) {
      mdl_close.forEach(mlc => {
        mlc.addEventListener('click', e => {
          ...
          if (_mdls.length < 2) document.documentElement.classList.remove('--modal_showed');
    ...
    document.body.addEventListener('click', event => {
      ...
      if (el.classList.contains('modal') && el.classList.contains('--is_show')) {
        ...
        if (swd_mdls.length < 2) document.documentElement.classList.remove('--modal_showed');
      }
    });
  }
...
ready(() => {
  ...
  let productInfoSliderThumb = Site.initSplideSliders(".product__info-slider-thumb", {
    fixedHeight: "55px",
    fixedWidth: "75px",
    perPage: 1,
    perMove: 1,
  ...
  let productInfoSliderMain = Site.initSplideSliders(".product__info-slider-main", {
    type: "fade",
    perPage: 1,
    perMove: 1,
    height: "350px",
  ...
  }, null, productInfoSliderThumb);
  if (!Site.empty(productInfoSliderMain) && !Site.empty(productInfoSliderThumb)) {
